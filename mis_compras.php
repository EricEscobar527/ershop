...
<div class="logo">
   ...
   <div class="container_mis_compras"> 
   <?php 
            ...
            $consulta = "SELECT productos.imagen, productos.nombre, productos.precio, productos.id, DATE_FORMAT(fecha, '%d/%m/%y') AS fecha FROM productos INNER JOIN compras ON productos.id = compras.id_productos WHERE compras.id_sesion = '$correo'";           
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
            ...
            while($row = $resultado->fetch(PDO::FETCH_ASSOC)){    
                ...
                $id = $row['id'];
              ?> 
   <div class="mis_compras_img_nom">
       <div class="img_nom_mis_compras">
<div class=""><?php echo '<img class="img_mis_compras" src="data:image/jpeg;base64,'.base64_encode($imagen).'"/>'; ?></div>
<div class=""><?php echo "<p>$nombre</p>"?></div>
</div>
<div class="precio_fecha_mis_compras">
<div class="">Precio:<?php echo "<p>$ $precio,00</p>"?></div>
<h4>Fecha de compra</h4>
<div><?php echo "<p> $fecha</p>"?></div>

<form action="validar_compra.php" method="POST">
                <input type="hidden" name="id_producto" value="<?php echo $row['id'];?>">
                <input class="btn_compra_de_nuevo" type="submit" value="Comprar de nuevo">
            </form>
