...
<?php
            ...
            $consulta = "SELECT * FROM productos WHERE categoria = 'alimentos'";
    ...
    <form action="validar_compra.php" method="POST">
     ...
     if ($carrito_ids){?>
        <form method="POST" action="eliminar_carrito_alimentos.php?correo=<?php echo $_SESSION['correo'];?>">
                <input type="hidden" name="id_producto" value="<?php echo $row['id'];?>">
                <span class="btn_en_el_carrito">En el carrito</span>
                <input class="btn_carrito" type="submit" value="Quitar">
            </form>
<?php }else { ?>
        <form method="POST" action="agregar_carrito_alimentos.php?correo=<?php echo $_SESSION['correo'];?>">
                <input type="hidden" name="id_producto" value="<?php echo $row['id'];?>">
