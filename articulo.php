   ...
   <div class="table-area" id="tabla_resultados">
    ...
    <?php
           ...
           $resultado->execute();
            ...
            while($row = $resultado->fetch(PDO::FETCH_ASSOC)){    
            ?>
        <div ><?php echo '<img class="articulo_img" src="data:image/jpeg;base64,'.base64_encode($row['imagen']).'"/>'; ?></div>
        <div class="articulo_info">
        <div class="articulo_nombre"><?php echo $row['nombre'];?></div>
        <div class="articulo_descripcion"><?php echo $row['descripcion'];?></div>
        <div class="articulo_precio">$<?php echo $row['precio'];?>,00</div>
        
        <div class="articulo_btn">
     ...
     if ($carrito_ids){?>
        <form method="POST" action="eliminar_carrito.php?correo=<?php echo $_SESSION['correo'];?>">
                <input type="hidden" name="id_producto" value="<?php echo $row['id'];?>">
                <span class="btn_en_el_carrito">En el carrito</span>
                <input class="btn_carrito" type="submit" value="Quitar">
            </form>
<?php }else { ?>
        <form method="POST" action="agregar_carrito.php?correo=<?php echo $_SESSION['correo'];?>">
                <input type="hidden" name="id_producto" value="<?php echo $row['id'];?>">
